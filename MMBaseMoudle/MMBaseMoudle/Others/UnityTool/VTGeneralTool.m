...
+ (void)showSettingAlertStr:(NSString *)tipStr{
    ...
    if (floor(NSFoundationVersionNumber) > NSFoundationVersionNumber_iOS_7_1) {
        [[[HYAlertView alloc] initWithTitle:@"提示" icon:nil message:tipStr block:^(HYAlertView *alertView, NSInteger buttonIndex) {
            if (buttonIndex) {
...
{
//    NSString * str = [date stringYearMonthDayCompareToday];
//    NSDateFormatter *objDateformat = [[NSDateFormatter alloc] init];
//    [objDateformat setDateFormat:@"yyyy-MM-dd HH:mm:ss"];
//    NSString * timeStr = [NSString stringWithFormat:@"%@",[objDateformat stringFromDate: date]];
//    if ([str isEqualToString:@"今天"])
//    {
//        return [timeStr substringWithRange:NSMakeRange(10, 6)];
//    }
//    else if ([str isEqualToString:@"昨天"])
//    {
//        return [NSString stringWithFormat:@"昨天 %@",[timeStr substringWithRange:NSMakeRange(10, 6)]];
//    }
//    else
//    {
//        return timeStr;
...
+ (NSString *)setTimeLength:(NSInteger)timeLength
{
    ...
    int times = time - timeLength;
    int Minute = times/60;
    int Hour  = times/(60*60);
    int Day = times/(60*60*24);
    int Month = times/(60*60*24*30);
    NSString * titleString = nil;
    if (Month>0) {
        titleString = [NSString stringWithFormat:@"%d月前",Month];
        return titleString;
    }else if(Day>0){
        titleString = [NSString stringWithFormat:@"%d天前",Day];
        return titleString;
    }else if(Hour>0){
        titleString = [NSString stringWithFormat:@"%d小时前",Hour];
        return titleString;
    }else if(Minute>0){
        titleString = [NSString stringWithFormat:@"%d分钟前",Minute];
        return titleString;
    }else{
        titleString = [NSString stringWithFormat:@"%d秒前",times];
        return titleString;
    }
}
...
{
    CGRect ContentRect = [title boundingRectWithSize:CGSizeMake(width, 1000) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:font} context:nil];
