...
- (void)viewDidLoad {
    ...
    /*
    [self.tableView registerClass:[SearchListTCell class] forCellReuseIdentifier:kSearchListTCelled];
    [self.tableView registerClass:[BabyCollectionTCell class] forCellReuseIdentifier:kBabyCollectCelled];
    */
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    id sData = self.searchSuggestions[indexPath.row];
    if ([self.searchSuggestions count]) {
        /*
        id object = [self.searchSuggestions firstObject];
        if ([object isKindOfClass:[Person class]]) {
             SearchListTCell *sCell = [tableView dequeueReusableCellWithIdentifier:kSearchListTCelled forIndexPath:indexPath];
            //[tableView addCellLine:sCell forRowAtIndexPath:indexPath];
            sCell.person = (Person *)sData;
            return sCell;
        }
        if ([object isKindOfClass:[GoodsContent class]]) {
            BabyCollectionTCell *gCell = [tableView dequeueReusableCellWithIdentifier:kBabyCollectCelled forIndexPath:indexPath];
            [tableView addCellLine:gCell forRowAtIndexPath:indexPath];
            gCell.gContent = (GoodsContent *)sData;
            gCell.joinGoodsCarBlock = ^(GoodsContent *model) {
                [NetworkRequest requestShopCartWith:model.id good_id:model.goods_id success:^(NSURLSessionDataTask *task, NSDictionary *responseObject) {
                }];
            };
            return gCell;
        }*/
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.didSelectCellBlock) self.didSelectCellBlock([tableView cellForRowAtIndexPath:indexPath], self.searchSuggestions[indexPath.row]);
}

- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView
{
    if (_firstResponderBlock) {
        _firstResponderBlock();
    }
}
