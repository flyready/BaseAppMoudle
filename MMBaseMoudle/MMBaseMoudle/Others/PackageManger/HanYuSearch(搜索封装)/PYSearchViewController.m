 ...
 */
@property (nonatomic, assign) BOOL didClickSuggestionCell, showOnce;
...
@property (nonatomic, strong) TrackContentView *trackView;
...
- (void)viewWillDisappear:(BOOL)animated
{
    ...
    [[UIApplication sharedApplication] setStatusBarStyle:(self.searchType != kSearchGoods)];
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    UINavigationBar *navBar = self.navigationController.navigationBar;
    UIImage *image = [UIImage imageWithColor:ViewBackColor
                            size:CGSizeMake(kScreenW, kNavigationBarH)];
    ...
    if (NO == navBar.translucent) {
        ...
        if (!navBar.barTintColor) {
            navBar.barTintColor = PYSEARCH_COLOR(249, 249, 249);
        }
    }
    self.searchBar.text = self.searchText;
    //DLog(@"  - - - - - -- - - -self.searchText    %@",self.searchText);
}

- (void)viewDidAppear:(BOOL)animated  // 出现后
{
    [super viewDidAppear:animated];
    if (!_showOnce) {    //  保证只加载 一次
...
- (TrackContentView *)trackView
{
    if (!_trackView) {
        _trackView = [[TrackContentView alloc]
                        initWithFrame:self.view.bounds];
        [self.trackView setHidden:YES];
    }
    typeof(self) __weak weakSelf = self;
    _trackView.trackResultBlock = ^(NSString *searchText){
        [weakSelf saveSearchCacheAndRefreshView];
        if (weakSelf.didSearchBlock){
            weakSelf.didSearchBlock(weakSelf, nil, searchText);
        }
    };
    return _trackView;
}
...
- (PYSearchSuggestionViewController *)searchSuggestionVC
{
    if (!_searchSuggestionVC) {
        ...
        searchSuggestionVC.firstResponderBlock = ^{
            [_weakSelf.searchBar resignFirstResponder];
        };
        searchSuggestionVC.didSelectCellBlock = ^(UITableViewCell *didSelectCell, id data) {
...
#pragma clang diagnostic pop
                ...
                if (_swSelf.didReturnDataBlock) {
                    _swSelf.didReturnDataBlock(data, self);
                }else{
...
- (void)requestHotWords
{
    ...
    [parma setValue:[UserObject shareUser].hotel_id forKey:@"hotel_id"];
    //DLog(@" - - - - -- -- - - -  parma - - - %@",parma);
    [NetworkRequest GETRequest:HY_GoodsSearch parameters:parma success:^(NSURLSessionDataTask *task, NSDictionary *responseObject) {
        if (SUCCESS) {
            NSDictionary *reponData = responseObject[@"responseData"];
            SearchGoods *sGoods = [SearchGoods mj_objectWithKeyValues:reponData];
            self.hotSearches = sGoods.hot_words;
        }
...
- (void)setSearchType:(SearchType)searchType
{
    ...
    if (_searchType == kSearchGoods) {
#pragma 搜索热门关键字
        [self requestHotWords];
        NSString *cachePath = [NSString stringWithFormat:@"GoodsPYSearchhistories_%@.plist",@""];
        self.searchHistoriesCachePath = [[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) lastObject] stringByAppendingPathComponent:cachePath];
        // the path of search record cached
    }else if(_searchType == kSearchStore){
        self.searchHistoriesCachePath = [[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) lastObject] stringByAppendingPathComponent:@"StorePYSearchhistories.plist"];
    }
}
...
- (void)setup
{
    ...
    MyCustomSearchBar *searchBar = [[MyCustomSearchBar alloc]
                                initWithFrame:titleView.bounds];
    ...
    //[self.baseSearchTableView setRowHeight:kTCellHeight];
...
- (void)setSearchSuggestions:(NSArray<NSString *> *)searchSuggestions
{
    ...
    //[self.searchSuggestionVC.view setHidden:(!self.trackView.hidden)];
}
...
- (void)setHotSearches:(NSArray *)hotSearches
{
    ...
    if (0 == hotSearches.count || !self.showHotSearch) {
        //self.searchSuggestionView.frame = self.hotSearchTagsContentView.bounds;
    ...
    //_searchSuggestionVC.view.y = _hotSearchTagsContentView.maxY;
...
{
    WEAKSELF
    [[[HYAlertView alloc] initWithTitle:@"提示" icon:nil message:@"确定要清空历史记录吗?" block:^(HYAlertView *alertView, NSInteger buttonIndex) {
        if (buttonIndex) {
            [weakSelf emptySearchHistory];
        }
    } buttonTitles:@"取消", @"确定" ,nil] show];
}
...
{
#pragma 如果需要手下的话   dismiss viewcontroller
    if (self.needDismiss) { [self cancelDidClick]; }
    UISearchBar *searchBar = self.searchBar;
    [searchBar resignFirstResponder];
    NSString *searchText = self.searchText = searchBar.text;
...
- (CGFloat)searchSuggestionView:(UITableView *)searchSuggestionView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    /*
    if (self.searchType == kSearchGoods) {
        return kGoodCellH;
    }
    */
    return 44.f;
...
- (void)searchBar:(UISearchBar *)searchBar textDidChange:(NSString *)searchText
{
    ...
    self.trackView.searcText = self.searchText;
   
    if ([CustomUtils ISNULL:self.searchText]) {
        [self.trackView setHidden:YES];
    }else{
        [self.trackView setHidden:NO];
        [self.searchSuggestionVC.view setHidden:YES];
    }
    //  如果动了   清空搜索记录
