...
//  UIBadgeView.m
//
//  Created by YunShang Company on 16-8-6.
//  Copyright (c) 2014年 Coding. All rights reserved.
//

#define kMaxBadgeWith 100.0
#define kBadgeTextOffset 1.f
#define kBadgePading 1.f
...
+ (CGSize)badgeSizeWithStr:(NSString *)badgeValue font:(UIFont *)font{
    ...
    if (badgeSize.width <= badgeSize.height) {
        ...
        badgeSize = CGSizeMake(badgeSize.width+10.f, badgeSize.height);
    }
...
- (void)commonInitialization {
    ...
    _badgeBackgroundColor = [UIColor whiteColor];//UIColorFromRGB(0xff3b30);
    _badgeTextColor = WholeColor;//[UIColor whiteColor];
    if (IS_IPHONE_6 || IS_IPHONE_6P) {
    ...
    if ([[self badgeValue] length]) {
        ...
        if ([self badgeBackgroundColor]) {
            if (![self.badgeValue isEqualToString:kBadgeTipStr]) {//外白色描边
                CGContextSetFillColorWithColor(context, [WholeColor CGColor]);
                //[UIColor clearColor]
                if (badgeSize.width > badgeSize.height) {
...
- (CGRect)badgeFrameWithStr:(NSString *)badgeValue{
    ...
    CGRect badgeFrame = CGRectMake(0, 0, badgeSize.width+4*kBadgePading, badgeSize.height+4*kBadgePading);//8=2*2（红圈-文字）+2*2（白圈-红圈）
    return badgeFrame;
}
